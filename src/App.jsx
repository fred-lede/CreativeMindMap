...
import { 
  ...
  Sidebar as SidebarIcon, Plus, FileText, FolderOpen, MoreVertical, Layout, Check, AlertTriangle, Globe, Search, BrainCircuit, Lock, Circle, Square, MousePointer2, HelpCircle, Focus, LayoutGrid, Maximize, ScrollText, Palette, ExternalLink
} from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className="w-full h-screen overflow-hidden relative selection:bg-yellow-200" 
      ...
      {/* Toast */}
      {toast && (
        <div className={`fixed top-6 left-1/2 -translate-x-1/2 z-[70] px-4 py-3 rounded-xl shadow-2xl backdrop-blur-md border flex items-center gap-3 animate-in fade-in duration-300 ${toast.type === 'error' ? 'bg-red-50/90 border-red-200 text-red-700' : 'bg-green-50/90 border-green-200 text-green-700'}`}>
      ...
      {/* Ollama Help Modal */}
      {showOllamaHelp && (
          <div className="fixed inset-0 z-[100] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowOllamaHelp(false)} onWheel={e=>e.stopPropagation()} onMouseDown={e=>e.stopPropagation()}>
      ...
      {showSearchSources && searchSources.length > 0 && (
          <div className="fixed inset-0 z-[100] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowSearchSources(false)} onWheel={e=>e.stopPropagation()} onMouseDown={e=>e.stopPropagation()}>
      ...
      {/* Sidebar */}
      <div 
        className={`fixed top-0 left-0 h-full z-[60] backdrop-blur-xl border-r shadow-2xl transition-all duration-300 ease-in-out flex flex-col ${isSidebarOpen ? 'w-72 translate-x-0' : 'w-72 -translate-x-full pointer-events-none'}`}
      ...
      <div className="fixed inset-0 pointer-events-none z-50">
         ...
         {showControls && (
           <div className="absolute bottom-8 left-1/2 -translate-x-1/2 w-full max-w-xl px-4 flex flex-col gap-3 items-center pointer-events-none">
               {/* Action Buttons */}
         ...
         <div className="absolute bottom-8 right-8 flex flex-col gap-2 pointer-events-auto" onMouseDown={e=>e.stopPropagation()}>
      ...
      {showClearConfirm && <div className="fixed inset-0 z-[100] bg-black/20 backdrop-blur-sm flex items-center justify-center p-4" onMouseDown={e=>e.stopPropagation()} onWheel={e=>e.stopPropagation()}><div className="p-6 rounded-2xl shadow-xl max-w-sm" style={{ backgroundColor: currentTheme.bg, color: currentTheme.text, border: `1px solid ${currentTheme.glassBorder}` }} onClick={e=>e.stopPropagation()}><h3 className="font-bold text-red-600 flex gap-2 mb-2"><AlertTriangle/>{t.confirmClearTitle}</h3><p className="text-sm opacity-80 mb-4">{t.confirmClearText}</p><div className="flex justify-end gap-2"><button onClick={()=>setShowClearConfirm(false)} className="px-4 py-2 rounded-lg opacity-70 hover:opacity-100" style={{ backgroundColor: currentTheme.line }}>{t.cancel}</button><button onClick={executeNewProject} className="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600">{t.confirm}</button></div></div></div>}
      
      {/* 新增：專案刪除確認 Modal */}
      {showDeleteProjectConfirm && (
        <div className="fixed inset-0 z-[100] bg-black/20 backdrop-blur-sm flex items-center justify-center p-4" onMouseDown={e=>e.stopPropagation()} onWheel={e=>e.stopPropagation()}>
